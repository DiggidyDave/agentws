...
# ]
#
# A profile named "default" is used when 'create' gets no -p/--project.
# Profiles may have an empty repos list — workspaces start empty and are
# populated later with 'agentws add'.
#
# [profiles.default]
# repos = []
...
def normalize_repo_url(arg: str) -> str:
    """Expand scheme-less host shorthand (github.com/acme/foo) to an https URL.

    Local paths and real URLs (scheme, scp-style git@host:...) pass through.
    """
    host = arg.split("/", 1)[0]
    if (
        "/" in arg
        and "://" not in arg
        and not arg.startswith(("git@", "/", ".", "~"))
        and "." in host
        and not Path(arg).expanduser().exists()
    ):
        return f"https://{arg}"
    return arg
...
def repo_arg_to_spec(
    ...
    """Turn a CLI repo argument (bare name, host/owner/repo, or git URL) into a RepoSpec."""
    if arg.startswith(("git@", "https://", "http://", "ssh://", "file://")) or "/" in arg:
        name = arg.rstrip("/").split("/")[-1].removesuffix(".git")
        return RepoSpec(
            name=name, url=normalize_repo_url(arg), base=base, clone=clone
        )
    if not org:
        raise Fail(
...
{rows}"""
        ...
        repos_section = (
            "(No repos yet — add them with `agentws add <repo-or-url>...` "
...
Multi-repo workspace generated by `agentws`{f" from profile `{profile}`" if profile else ""}.
...
- `agentws status {name}` shows per-repo dirty/pushed state across the
  whole workspace; `agentws update {name}` fetches origin and rebases every
  repo onto its base branch; `agentws rm {name}` tears it down safely.
"""
...
def create(
    ...
    repo: list[str] = typer.Option(
        None, "-r", "--repo",
        help="Repo(s) to include in addition to the profile's (repeatable). "
             "Accepts a bare name (expanded via defaults.org), "
             "host/owner/repo like github.com/acme/foo, or a full git URL. "
             "With --repo, the profile is optional.",
    ),
    ...
    if not profile and "default" in cfg.get("profiles", {}):
        profile = "default"
    if not profile and not repo and existing is None:
        raise Fail(
            "No profile or repos: pass -p/--profile and/or --repo, use "
            "--project with a project that defines a default profile, or "
            "define a profile named 'default' (agentws profile add default) "
            "to fall back on."
        )
    specs = resolve_repos(cfg, profile) if profile else []
    org = cfg.get("defaults", {}).get("org", "")
    covered_names = {s.name for s in specs}
    for arg in repo or []:
        spec = repo_arg_to_spec(arg, org, clone, base)
        if spec.name in covered_names:
            continue
        covered_names.add(spec.name)
        specs.append(spec)
    ...
    if repos_meta or not failures:
        write_workspace_meta(
            ws_dir, name, branch, profile or "", description, repos_meta, project
        )
        write_claude_md(
            ws_dir, name, branch, profile or "", description, repos_meta,
...
def add(
    repos: list[str] = typer.Argument(
        ..., help="Repo names (org-expanded), host/owner/repo shorthand "
                  "(github.com/acme/foo), or full git URLs."
    ),
...
def list_workspaces() -> None:
    ...
    for ws_dir in sorted(p for p in root.iterdir() if p.is_dir()):
        ...
        info(
            f"{meta['name']:<24} profile={meta.get('profile') or '-':<16} "
...
def status(
    ...
    proj = f", project={meta['project']}" if meta.get("project") else ""
    info(f"Workspace {name} (profile={meta.get('profile') or '-'}{proj})"
         + (f": {desc}" if desc else ""))
    for r in meta.get("repos", []):
...
def update(
    ...
    autostash: bool = typer.Option(
        False, "--autostash",
        help="Stash uncommitted changes around the rebase instead of "
             "skipping dirty repos.",
    ),
) -> None:
    """Update a workspace: fetch origin and rebase each repo onto its base branch.

    Repos with uncommitted changes are skipped unless --autostash. A rebase
    that hits conflicts is aborted, leaving that repo untouched for you to
    rebase manually.
    """
    if name:
        cfg = load_config()
    ...
    for r in meta.get("repos", []):
        repo_dir = ws_dir / r["name"]
        if not repo_dir.exists():
            warn(f"  {r['name']:<28} MISSING (re-run create to restore); skipped")
            continue
        try:
            git("fetch", "--prune", "origin", cwd=repo_dir)
            base = r.get("base") or ""
            if not base or base == "?":
                if git_ok(
                ...
                else:  # worktree of a bare clone: no origin/HEAD symref
                    common = Path(git("rev-parse", "--git-common-dir", cwd=repo_dir))
                    base = repo_default_branch(common)
            if not git_ok(
                "rev-parse", "--verify", "--quiet", f"refs/remotes/origin/{base}",
                cwd=repo_dir,
            ):
                warn(f"  {r['name']:<28} base 'origin/{base}' not found; skipped")
                continue
            behind = int(git(
                "rev-list", "--count", f"HEAD..origin/{base}", cwd=repo_dir
            ))
            if behind == 0:
                info(f"  {r['name']:<28} up to date with origin/{base}")
                continue
            state = repo_state(repo_dir)
            if state.dirty and not autostash:
                warn(
                    f"  {r['name']:<28} {state.dirty} uncommitted change(s); "
                    "skipped (commit them or use --autostash)"
                )
                failures.append(r["name"])
                continue
            rebase_args = ["rebase"]
            if autostash:
                rebase_args.append("--autostash")
            rebase_args.append(f"origin/{base}")
            try:
                git(*rebase_args, cwd=repo_dir)
            except Fail:
                git_ok("rebase", "--abort", cwd=repo_dir)
                warn(
                    f"  {r['name']:<28} rebase onto origin/{base} hit "
                    "conflicts; aborted — rebase manually in that repo"
                )
                failures.append(r["name"])
                continue
            note = ""
            if state.has_upstream:
                note = (f" — branch was already pushed; publish with "
                        f"'git push --force-with-lease'")
            ok(f"  {r['name']:<28} rebased onto origin/{base} "
               f"({behind} new commit(s)){note}")
        except Fail as exc:
            failures.append(r["name"])
            err(f"  {r['name']}: FAILED\n{exc}")

    if failures:
        err(f"\n{len(failures)} repo(s) need attention: {', '.join(failures)}")
        raise typer.Exit(1)
...
def profile_add(
    ...
    repos: list[str] = typer.Argument(
        None, help="Repo names (or full git URLs). Omit for an empty profile."
    ),
    description: str = typer.Option("", "-d", "--description"),
) -> None:
    """Add a new profile (no repos = empty profile; fill it in later)."""
    repos = repos or []
    cfg = load_config()
    ...
    for r in repos:
        if r.startswith(("git@", "https://", "http://", "ssh://", "file://")) or "/" in r:
            item = tomlkit.inline_table()
            item["name"] = r.rstrip("/").split("/")[-1].removesuffix(".git")
            item["url"] = normalize_repo_url(r)
            entries.append(item)
        else:
            entries.append(r)
    table["repos"] = entries
